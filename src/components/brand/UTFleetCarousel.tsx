...
import { UT_CAROUSEL_SLIDES } from '@/lib/ut-carousel'

const INTERVAL = 3000
const SWIPE_THRESHOLD = 50

export default function UTFleetCarousel({ accent }: { accent: string }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      setActiveIndex((i) => (i + 1) % UT_CAROUSEL_SLIDES.length)
    }, INTERVAL)
    return () => clearTimeout(t)
  }, [activeIndex, isPaused, reducedMotion])

  const goPrev = useCallback(() => {
    setActiveIndex((i) => (i - 1 + UT_CAROUSEL_SLIDES.length) % UT_CAROUSEL_SLIDES.length)
  }, [])
    ...
    >
      <div className="isuzu-carousel-inner">
        <div className="isuzu-carousel-header">
          ...
          <h2 className="isuzu-carousel-heading">Für jede Aufgabe der passende Aufbau</h2>
        </div>
        ...
        <div
          ...
          aria-label="UT-Aufbauten im Überblick"
        ...
        >
          <div className="isuzu-carousel-track" aria-live="polite">
            {UT_CAROUSEL_SLIDES.map((slide, idx) => {
              ...
              return (
                <article
                  ...
                  aria-label={`${idx + 1} von ${UT_CAROUSEL_SLIDES.length}: ${slide.title}`}
                >
          ...
          <button type="button" className="isuzu-carousel-arrow isuzu-carousel-arrow--prev" onClick={goPrev} aria-label="Vorheriger Aufbau">
            <ChevronLeft size={20} />
          </button>
          <button type="button" className="isuzu-carousel-arrow isuzu-carousel-arrow--next" onClick={goNext} aria-label="Nächster Aufbau">
