...
import { PIAGGIO_CAROUSEL_SLIDES } from '@/lib/piaggio-carousel'

const INTERVAL = 3000
const SWIPE_THRESHOLD = 50

export default function PiaggioFleetCarousel({ accent }: { accent: string }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      setActiveIndex((i) => (i + 1) % PIAGGIO_CAROUSEL_SLIDES.length)
    }, INTERVAL)
    return () => clearTimeout(t)
  }, [activeIndex, isPaused, reducedMotion])

  const goPrev = useCallback(() => {
    setActiveIndex((i) => (i - 1 + PIAGGIO_CAROUSEL_SLIDES.length) % PIAGGIO_CAROUSEL_SLIDES.length)
  }, [])
    ...
    >
      <div className="isuzu-carousel-inner">
        ...
        <div
          ...
          aria-label="Piaggio-Flotte im Überblick"
        ...
        >
          <div className="isuzu-carousel-track" aria-live="polite">
            {PIAGGIO_CAROUSEL_SLIDES.map((slide, idx) => {
              ...
              return (
                <article
                  ...
                  aria-label={`${idx + 1} von ${PIAGGIO_CAROUSEL_SLIDES.length}: ${slide.title}`}
                >
