...
import { ISUZU_CAROUSEL_SLIDES } from '@/lib/isuzu-carousel'

const INTERVAL = 3000
const SWIPE_THRESHOLD = 50

export default function IsuzuFleetCarousel({ accent }: { accent: string }) {
  ...
  // Auto-advance via setTimeout keyed on activeIndex — naturally resets on
  // manual navigation.
  useEffect(() => {
    if (isPaused || reducedMotion) return
    const t = setTimeout(() => {
      setActiveIndex((i) => (i + 1) % ISUZU_CAROUSEL_SLIDES.length)
    }, INTERVAL)
    return () => clearTimeout(t)
  }, [activeIndex, isPaused, reducedMotion])

  const goPrev = useCallback(() => {
    setActiveIndex((i) => (i - 1 + ISUZU_CAROUSEL_SLIDES.length) % ISUZU_CAROUSEL_SLIDES.length)
  }, [])
    ...
    >
      <div className="isuzu-carousel-inner">
        ...
        <div
          ...
          aria-label="Isuzu-Flotte im Überblick"
        ...
        >
          <div className="isuzu-carousel-track" aria-live="polite">
            {ISUZU_CAROUSEL_SLIDES.map((slide, idx) => {
              ...
              return (
                <article
                  ...
                  aria-label={`${idx + 1} von ${ISUZU_CAROUSEL_SLIDES.length}: ${slide.title}`}
                >
          ...
          <button
            type="button"
            className="isuzu-carousel-arrow isuzu-carousel-arrow--prev"
            onClick={goPrev}
          ...
          <button
            type="button"
            className="isuzu-carousel-arrow isuzu-carousel-arrow--next"
            onClick={goNext}
