    ...
    <PageWrapper>
      <div className="p-6 lg:p-8 max-w-2xl mx-auto">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-6">
          <h3 className="text-xs font-semibold text-amber-800 uppercase tracking-wider mb-4">
            Ersteinrichtung Datenbank
          </h3>
          <div className="space-y-2">
            {['npx prisma generate', 'npx prisma db push', 'npx tsx prisma/seed.ts'].map((cmd) => (
              <code key={cmd} className="block bg-amber-100 text-amber-900 text-xs px-3 py-2 rounded-lg font-mono">
                {cmd}
              </code>
            ))}
          </div>
