...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const product = await client.fetch(
    `*[_type == "product" && slug.current == $productSlug && brand->slug.current == $brandSlug && brand->center->slug.current == "nutzfahrzeugcenter"][0]{
  ...
  return {
    ...
    description: `${product.name} von ${product.brandName} beim Nutzfahrzeugcenter Ernst Moser GmbH, Gerlafingen.`,
  }
}

export async function generateStaticParams() {
  const products = await client.fetch<Array<{ brand: string; product: string }>>(
    `*[_type == "product" && brand->center->slug.current == "nutzfahrzeugcenter"] {
...
export default async function NutzfahrzeugProductPage({ params }: Props) {
